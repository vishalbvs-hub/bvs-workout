      ...
      {showGuide && guide && (
        <div style={{ background: "#0f172a", borderRadius: 14, padding: 16, marginBottom: 16, border: "1px solid #1e293b" }}>
          ...
          {videoId ? (
            ...
            <div style={{ marginBottom: 16 }}>
              <button onClick={() => window.open("https://www.youtube.com/results?search_query=" + encodeURIComponent(guide.yt + " Jeff Nippard form tutorial"), "_blank")} style={{ display: "flex", alignItems: "center", gap: 10, background: "#dc2626", borderRadius: 10, padding: "12px 16px", border: "none", cursor: "pointer", width: "100%", marginBottom: 8 }}>
                <span style={{ fontSize: 20 }}>▶️</span>
                <div style={{ textAlign: "left" }}><div style={{ color: "#fff", fontSize: 14, fontWeight: 700 }}>Find Form Video</div><div style={{ color: "rgba(255,255,255,0.7)", fontSize: 11 }}>Opens YouTube — save one to embed it here</div></div>
              </button>
              <button onClick={() => setShowVideoEdit(!showVideoEdit)} style={{ background: "none", border: "none", color: "#60a5fa", fontSize: 12, cursor: "pointer", padding: 2 }}>{showVideoEdit ? "Cancel" : "Paste a YouTube link to embed it"}</button>
            </div>
          )}
          {/* Custom video input */}
          {showVideoEdit && (
  ...
  return (
    <div style={{ background: "#030712", minHeight: "100vh", color: "#f8fafc", fontFamily: "'DM Sans', sans-serif", paddingBottom: 80 }}>
      ...
      {view === "dashboard" && <div style={{ padding: 20 }}>
        ...
        <div style={{ ...cS, marginTop: 12 }}><span style={{ color: "#94a3b8", fontSize: 13, fontWeight: 600, letterSpacing: 1 }}>🏋️ TODAY'S WORKOUT</span>{twt ? <div style={{ marginTop: 10 }}><div style={{ color: "#22c55e", fontSize: 16, fontWeight: 700 }}>✓ {getWorkout(twt)?.name || twt} Completed</div><button onClick={() => startW(twt)} style={{ ...bP, background: "#1e293b", marginTop: 10, fontSize: 14 }}>View / Edit →</button></div> : iwd() ? <div style={{ marginTop: 10 }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: 18 }}>{WORKOUTS[gnw()]?.icon}</span><div style={{ color: "#f8fafc", fontSize: 16, fontWeight: 700 }}>{WORKOUTS[gnw()].name}</div></div><div style={{ color: "#64748b", fontSize: 13, marginBottom: 12, marginTop: 4 }}>{WORKOUTS[gnw()].subtitle}</div><button onClick={() => startW(gnw())} style={bP}>Start Workout →</button><button onClick={skipWorkout} style={{ background: "none", border: "none", color: "#64748b", fontSize: 13, cursor: "pointer", marginTop: 10, width: "100%", padding: 8 }}>Skip — do this one tomorrow instead</button></div> : isAbDay() ? <div style={{ marginTop: 10 }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: 18 }}>🟠</span><div style={{ color: "#f97316", fontSize: 16, fontWeight: 700 }}>{AB_WORKOUTS[gnaw()]?.name}</div></div><div style={{ color: "#64748b", fontSize: 13, marginBottom: 12, marginTop: 4 }}>{AB_WORKOUTS[gnaw()]?.subtitle} • At home with dumbbells, bench & medicine ball</div><button onClick={() => startW(gnaw())} style={{ ...bP, background: "linear-gradient(135deg, #f97316, #ea580c)" }}>Start Ab Workout →</button><div style={{ borderTop: "1px solid #1f2937", marginTop: 12, paddingTop: 10 }}><button onClick={workOutToday} style={{ background: "none", border: "1px solid #334155", color: "#94a3b8", borderRadius: 10, padding: "10px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer", width: "100%" }}>Skip abs — do {WORKOUTS[gnw()]?.name} instead</button></div></div> : <div style={{ marginTop: 10, color: "#64748b", fontSize: 14 }}>Loading...</div>}<button onClick={realignSchedule} style={{ background: "none", border: "none", color: "#475569", fontSize: 11, cursor: "pointer", marginTop: 8, width: "100%", padding: 4 }}>🔄 Realign schedule from today</button></div>
